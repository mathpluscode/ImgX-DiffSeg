...
def batch_patch_random_sample(
    ...
    def slice_per_sample(
        x: jnp.ndarray, start_indices_i: jnp.ndarray
    ) -> jnp.ndarray:
        """Slice per sample in the batch.
...
def get_patch_grid(
    ...
    """
    indices = []
    for img_size_dim, patch_size_dim, ovlp_size_dim in zip(
        image_shape, patch_shape, patch_overlap
    ):
    ...
    return np.stack(np.meshgrid(*indices, indexing="ij"), axis=-1).reshape(
        -1, len(image_shape)
    )
