...
class TestBatchPatchRandomSample(chex.TestCase):
    ...
    def test_shapes(
        ...
        got = self.variant(
            partial(batch_patch_random_sample, patch_shape=patch_shape)
        )(
            key=key,
...
class TestBatchPatchGridSample(chex.TestCase):
    ...
    def test_shapes(
        ...
        agg_x = self.variant(
            partial(batch_patch_grid_mean_aggregate, image_shape=image_shape)
        )(
            x_patch=got,
            start_indices=start_indices,
        )
...
class TestAddPatchWithChannel(chex.TestCase):
    ...
    def test_values(
        ...
        count_patch = jax.lax.dynamic_slice(
            count,
