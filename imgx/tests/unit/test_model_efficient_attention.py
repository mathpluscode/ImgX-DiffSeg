...
from imgx.model.efficient_attention import (
    ...
    attention_with_key_value_chunk,
    attention_with_qkv_chunks,
    summarize_chunk,
)
...
def hk_mha_attention(
    ...
    """Multi-head attention from hk.MultiHeadAttention without mask.
...
class TestSummarizeChunk(chex.TestCase):
    ...
    def test_shapes(
        ...
        # check shapes
        chex.assert_shape(
            got_attn, (self.batch_size, num_q, self.num_heads, v_features)
        )
        chex.assert_shape(
            got_attn_weights, (self.batch_size, num_q, self.num_heads)
        )
        chex.assert_shape(
            got_max_logits, (self.batch_size, num_q, self.num_heads)
        )


class TestAttentionWithKVChunk(chex.TestCase):
    """Test the function attention_with_key_value_chunk."""
    ...
    def test_shapes(
        ...
        # check shape
        chex.assert_shape(
            got, (self.batch_size, num_q, self.num_heads, v_features)
        )
...
class TestAttentionWithQKVChunks(chex.TestCase):
    """Test the function attention_with_query_chunk."""
...
class TestEfficientMultiHeadAttention(chex.TestCase):
    """Test the class EfficientMultiHeadAttention."""
    ...
    def test_shapes(
        ...
        query = jax.random.uniform(
            rng_query,
            shape=(self.batch_size, num_q, self.model_size),
        )
        key = jax.random.uniform(
            rng_key,
            shape=(self.batch_size, num_kv, self.model_size),
        )
        value = jax.random.uniform(
            rng_value,
            shape=(self.batch_size, num_kv, self.model_size),
        ...
        # check shape
        chex.assert_shape(got, (self.batch_size, num_q, self.model_size))

        # check values
        expected = forward_expected(query, key, value, mask)
        chex.assert_tree_all_close(got, expected, atol=1e-5, rtol=1e-5)
