...
class TestImageIterator(chex.TestCase):
    ...
    def test_output_shape_variants(
        ...
        def get_batch() -> (
            tuple[
                DatasetIterator, chex.ArrayTree, chex.ArrayTree, chex.ArrayTree
            ]
        ):
        ...
        assert dataset.num_valid_steps == int(
            np.ceil(max_num_samples_per_split / batch_size)
        )
...
class TestImageShape(chex.TestCase):
    ...
    def test_labels(
        ...
        if len(err_paths) > 0:
            raise ValueError(
                f"{err_paths} have less than {num_classes} "
