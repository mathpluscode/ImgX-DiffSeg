...
def save_uids(
    ...
    """
    pd.DataFrame({"uid": train_uids}).to_csv(
        out_dir / "train_uids.csv", index=False
    )
    pd.DataFrame({"uid": valid_uids}).to_csv(
        out_dir / "valid_uids.csv", index=False
    )
    pd.DataFrame({"uid": test_uids}).to_csv(
        out_dir / "test_uids.csv", index=False
    )
