...
from imgx_datasets.util import (
...
def n4_bias_field_correction(
    ...
    if shrink_factor > 1:
        image_volume = sitk.Shrink(
            image_volume, [shrink_factor] * image_volume.GetDimension()
        )
        mask_volume = sitk.Shrink(
            mask_volume, [shrink_factor] * image_volume.GetDimension()
        )
...
def load_and_preprocess_image_and_label(
    ...
    if len(target_spacing) != 3:
        raise ValueError(
            f"Target spacing should have 3 elements, got {target_spacing}."
        )
    if len(target_shape) != 3:
        raise ValueError(
            f"Target shape should have 3 elements, got {target_shape}."
        )
