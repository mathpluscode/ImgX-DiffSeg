...
def save_2d_segmentation_prediction(
    ...
    """
    if preds.ndim != 3:
        raise ValueError(f"Prediction should be 3D, but {preds.ndim}D is given.")
...
def save_3d_segmentation_prediction(
    ...
    """
    if preds.ndim != 4:
        raise ValueError(f"Prediction should be 4D, but {preds.ndim}D is given.")
    out_dir.mkdir(parents=True, exist_ok=True)
    for i, uid in enumerate(uids):
...
def load_3d_image(
    ...
    Returns:
        mask: (depth, height, width), the values are integers.
    """
    return np.asarray(sitk.GetArrayFromImage(sitk.ReadImage(image_path)), dtype=dtype)
